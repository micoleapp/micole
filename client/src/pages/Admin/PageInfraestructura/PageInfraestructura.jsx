...
export default function PageInfraestructura() {
  ...
  const handleDeleteInfra = (id) => {
    try {
      ...
      .then(res=>{
        if(res.isConfirmed){
          axios.delete('/infraestructuras/' + id)
          .then(res=>{
            Swal.fire({
              icon: "success",
  ...
  const getElementsForPage = (elements, page, itemsPerPage, searchTerm, categoria) => {
    let filteredElements = filterElements(elements, searchTerm)
    if(categoria){
      filteredElements = filteredElements.filter((element) => element.InfraestructuraTipoId === categoria);
    }
  ...
  const filterElements = (elements, searchTerm) => {
    return elements.filter((element) => element.nombre_infraestructura.toLowerCase().includes(searchTerm.toLowerCase()));
  };
        ...
        <div className="flex justify-between items-center">
          <div className="flex items-center bg-white w-fit rounded-md h-min">
            <input
              ...
              class="p-2 outline-none rounded-md w-[400px]"
  ...
  <ListItemText primary="Todos" />
        ...
        {
          getElementsForPage(infraestructura,page,itemsPerPage,searchTerm,categoria)?.map((infra, index) => (
            <div className="flex flex-col gap-4 bg-white p-5 rounded-md shadow-md" key={index}>
              <div className="flex justify-around gap-4 items-center">
                <div className="flex items-center gap-4">
                  ...
                  <div className="flex flex-col gap-2 w-[300px] ">
                    ...
                    <h1 className="font-medium">{infra.nombre_infraestructura}</h1>
                  </div>
                  <div className="flex flex-col gap-2 w-[300px] ">
                  <h2>Categoria</h2>
                  <h1 className="font-medium">{infra.Infraestructura_tipo.infraestructura_tipo}</h1>
                  </div>
                </div>
                <div className="flex gap-4">
                  ...
                  <button
                    type="button"
                    // onClick={(e) => handleEditInfra(e, infra.infraestructuraId)}
                    className="p-2 flex font-medium mx-auto lg:mx-0 w-fit text-[#0061dd] rounded-md bg-[#0061dd]/20 disabled:line-through"
                  >
                    Editar
                  </button>
        ...
        <Pagination count={Math.ceil(filterElements(infraestructura, searchTerm).filter((element) => categoria ? element.InfraestructuraTipoId === categoria : true).length / itemsPerPage)} page={page} onChange={handleChangePage} />
        </div>
